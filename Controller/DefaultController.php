...
use Tom32i\TwitterBundle\Model\TwitterOAuth;
...
class DefaultController extends SuperController
{
    ...
    private function linkUser($connection, $access_token)
    {
        ...
        if ($current_user && is_a($current_user, 'Tom32i\UserBundle\Entity\User') && $current_user->isValid())
        {
            ...
            if($user)
            {
                return $this->fail('link','This Twitter account is already linked to a Keeptools account.');
            }
    ...
    private function logUser($connection, $access_token)
    {
        ...
        $msg = "This twitter account is not register on Keeptools. Would you like to ";
